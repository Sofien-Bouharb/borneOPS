...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        // Password setup link resend: 3 attempts per 10 minutes per TARGET user id (route param),
        // deliberately separate from forgot-password so an admin resend can't burn the target
        // user's own recovery quota.
        RateLimiter::for('password-setup-resend', function (Request $request) {
            $targetUserId = $request->route('user')?->id ?? 'unknown';
            return Limit::perMinutes(10, 3)->by("password-setup-resend:{$targetUserId}");
        });
    }
}
