...
@pytest.mark.asyncio
async def test_transaction_event_started_calls_bridge_with_evse_and_connector_id(charge_point):
...
@pytest.mark.asyncio
async def test_transaction_event_started_passes_none_connector_id_when_omitted(charge_point):
    with patch("app.charge_point_v201.bridge_client.send_start_transaction_external", new=AsyncMock()) as mock_call:
        await charge_point.on_transaction_event(
            event_type="Started",
...
@pytest.mark.asyncio
async def test_transaction_event_started_passes_none_evse_id_when_evse_missing_entirely(charge_point):
    with patch("app.charge_point_v201.bridge_client.send_start_transaction_external", new=AsyncMock()) as mock_call:
        await charge_point.on_transaction_event(
            event_type="Started",
            ...
            trigger_reason="Authorized",
            seq_no=0,
            transaction_info={"transaction_id": "CP201-TXN-005"},
            evse=None,
            meter_value=[{
                "timestamp": "2026-08-12T13:05:00Z",
                "sampled_value": [{"value": 100.0, "measurand": "Energy.Active.Import.Register"}],
            }],
        )
